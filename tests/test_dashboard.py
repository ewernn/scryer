...
async def test_run_page_idor_other_users_run_returns_404(
    ...
    user_b_workspace = (
        await http_session.execute(
            ...
            .select(__import__("scryer.server.models.auth", fromlist=["Workspace"]).Workspace)
            .where(
                __import__("scryer.server.models.auth", fromlist=["Workspace"]).Workspace.slug
                == ws_b
            )
        )
    ).scalar_one()
    proj_b = await create_project(
        ...
        name="t",
        owner_user_id=user_b_workspace.owner_user_id,
    )
